...
Rcpp::RObject assembleHessian(
  ...
  if(!onlyRandom && (N != Nfull)) {
    Rcpp::Rcout << "assembleHessian: N " << N << " Nfull " << Nfull << " Nrandom " << Nrandom << "\n";
  }

  const Rcpp::List outAll = onlyRandom?secondRandom:secondFull;
  const Rcpp::IntegerVector iAll = outAll["i"];
  const Rcpp::IntegerVector jAll = outAll["j"];
  const size_t sizeH = iAll.size();
