...
betai_names <- sparse.model.matrix(~ 0 + hum + pol1, data[1]) |> colnames()
beta_names <- lapply(cities, \(ct) paste0(substr(ct, 1, 3), ":", betai_names)) |> unlist()
X <- lapply(cities, \(ct){
  sparse.model.matrix(~ 0 + hum + pol1, data[city == ct],
...
gamma_rounding <- 2
pol1_bins <- round(data$pol1/gamma_rounding) |> range()
data[, pol1_bin := factor(round(pol1/gamma_rounding), levels = pol1_bins[1]:pol1_bins[2])]

gammai_names <- sparse.model.matrix(~ 0 + pol1_bin, data[1]) |> colnames()
gamma_names <- lapply(c("global", cities), \(ct) paste0(substr(ct, 1, 3), ":", gammai_names)) |> unlist()
A <- cbind(
  sparse.model.matrix(~ 0 + pol1_bin, data, drop.unused.levels = F),
  lapply(cities, \(ct){
    sparse.model.matrix(~ 0 + pol1_bin, data[city == ct], drop.unused.levels = F)
  }) |> .bdiag()
) |> as("dgTMatrix")
...
theta_true <- log(rexp(2*np, .01))  # Log precision for each pollutant (will be exp(theta) to scale Q)

gamma_split = c("pol1", "pol2")[1:np] |> sapply(\(x) length(grep(x, gammai_names))) |> rep(nc+1)
...
opt$par - (parameters_list |> unlist())[!is.na(unlist(map))]
