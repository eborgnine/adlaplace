...
{  //
   ...
   {  printOp<Base, RecBase>(
         ::RCppAD::cppad_trace_stream(), play, itr.op_index(), i_var, op_code, arg
      );
      ::RCppAD::cppad_trace_stream() << std::endl;
   }
   //
   ...
   {  if( variable_y[i] )
      {  for(size_t k = order_low; k < n_order; ++k)
         ...
         {  const addr_t* null_addr = static_cast<addr_t*>( nullptr );
            printOp<Base, RecBase>(
               ::RCppAD::cppad_trace_stream(), play, itr.op_index(), index_y[i], FunrvOp, null_addr
            );
            Base* yi_ptr          = taylor + index_y[i] * cap_order + 0;
            const Base* null_base = static_cast<Base*>( nullptr );
            printOpResult<Base>(
               ::RCppAD::cppad_trace_stream(), n_order, yi_ptr, 0, null_base
            );
            ::RCppAD::cppad_trace_stream() << std::endl;
         }
      }
