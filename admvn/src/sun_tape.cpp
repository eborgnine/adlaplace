...
SunDpAD make_sun_dp_ad(const std::vector<AD>& par) {
  ...
  const std::array<AD, 3> z_omega{par[6], par[7], par[8]};
  ChAD Bu = unit_row_chol3(z_omega);
  ...
  // L: all nine entries free (row-major)
  ChAD L = zeros_ad3();
  for (std::size_t i = 0; i < kSunD; ++i) {
    for (std::size_t j = 0; j < kSunD; ++j) {
  ...
  const std::array<AD, 3> z_gamma{par[18], par[19], par[20]};
  ChAD Br = unit_row_chol3(z_gamma);
