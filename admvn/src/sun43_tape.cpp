...
ChAD zeros_ad_dm() {
  return ChAD(kSun43D, std::vector<AD>(kSun43M, AD(0.0)));
}
...
struct Sun43DpAD {
  std::vector<AD> xi = std::vector<AD>(kSun43D, AD(0.0));
  ChAD Omega = zeros_ad(kSun43D);          // D x D
  ChAD Delta = zeros_ad_dm();              // D x M
  ChAD Gamma = zeros_ad(kSun43M);          // M x M
};

// Parameter order (length 29): xi(4), nu(4), z(21)
Sun43DpAD make_sun43_dp_hs_ad(const std::vector<AD>& par) {
  if (par.size() != kSun43NPar) {
    throw std::runtime_error("par must have length 29");
  }
  Sun43DpAD out;
  for (std::size_t i = 0; i < kSun43D; ++i) {
    out.xi[i] = par[i];
  }
  std::vector<AD> z(kSun43HsNFree);
  for (std::size_t i = 0; i < kSun43HsNFree; ++i) {
    z[i] = par[2 * kSun43D + i];
  }
  ChAD B = unit_row_chol_joint(z);
  ChAD J = zeros_ad(kSun43JointD);
  for (std::size_t i = 0; i < kSun43JointD; ++i) {
    for (std::size_t j = 0; j < kSun43JointD; ++j) {
      for (std::size_t k = 0; k < kSun43JointD; ++k) {
        J[i][j] += B[i][k] * B[j][k];
      }
    }
  }
  ...
  for (std::size_t i = 0; i < kSun43D; ++i) {
    const AD si = CppAD::CondExpGt(
      par[kSun43D + i], AD(1e-12), par[kSun43D + i], AD(1e-12));
    for (std::size_t j = 0; j < kSun43D; ++j) {
      const AD sj = CppAD::CondExpGt(
        par[kSun43D + j], AD(1e-12), par[kSun43D + j], AD(1e-12));
      out.Omega[i][j] = si * sj * J[i][j];
    }
  }
  // Delta = J_UV (D x M)
  for (std::size_t i = 0; i < kSun43D; ++i) {
    for (std::size_t j = 0; j < kSun43M; ++j) {
      out.Delta[i][j] = J[i][kSun43D + j];
    }
  }
  // Gamma = J_VV (M x M)
  for (std::size_t i = 0; i < kSun43M; ++i) {
    for (std::size_t j = 0; j < kSun43M; ++j) {
      out.Gamma[i][j] = J[kSun43D + i][kSun43D + j];
    }
  }
  return out;
}

Sun43DpAD make_sun43_dp_dispatch(
  const std::vector<AD>& par, Sun43ParMap par_map) {
  if (par_map == Sun43ParMap::kHyperspherical) {
    return make_sun43_dp_hs_ad(par);
  }
  throw std::runtime_error(
    "SUN(4,3) block-Cholesky parameterization is not implemented; use hs");
}
...
Mat sun43_sigma_double(
  const std::vector<double>& par, Mat& lambda, Sun43ParMap par_map) {
  std::vector<AD> par_ad(kSun43NPar);
  for (std::size_t i = 0; i < kSun43NPar; ++i) {
    par_ad[i] = AD(par[i]);
  }
  Sun43DpAD dp = make_sun43_dp_dispatch(par_ad, par_map);

  ChAD omega_bar = cov2cor(dp.Omega);
  ChAD omega_inv = zeros_ad(kSun43D);
  ChAD l{};
  chol_lower_ad(omega_bar, l);
  for (std::size_t col = 0; col < kSun43D; ++col) {
    std::vector<AD> y(kSun43D, AD(0.0));
    for (std::size_t i = 0; i < kSun43D; ++i) {
    ...
    for (int i = static_cast<int>(kSun43D) - 1; i >= 0; --i) {
      AD s = y[static_cast<std::size_t>(i)];
      for (std::size_t k = static_cast<std::size_t>(i) + 1; k < kSun43D; ++k) {
  ...
  ChAD d_oinv(kSun43M, std::vector<AD>(kSun43D, AD(0.0)));
  for (std::size_t i = 0; i < kSun43M; ++i) {
    for (std::size_t j = 0; j < kSun43D; ++j) {
      AD s = AD(0.0);
      for (std::size_t k = 0; k < kSun43D; ++k) {
  ...
  lambda = zeros_mat(kSun43M);
  for (std::size_t i = 0; i < kSun43M; ++i) {
    for (std::size_t j = 0; j < kSun43M; ++j) {
      lambda[i][j] = Value(lambda_ad[i][j]);
    }
  }
  Mat omega = zeros_mat(kSun43D);
  for (std::size_t i = 0; i < kSun43D; ++i) {
    for (std::size_t j = 0; j < kSun43D; ++j) {
...
AD log_mvn_obs(const std::vector<AD>& x, const Sun43DpAD& dp) {
  ChAD l{};
  ...
  return AD(-0.5 * static_cast<double>(kSun43D)) * AD(kLog2Pi)
    - AD(0.5) * log_det - AD(0.5) * quad;
}

ChAD lambda_from_dp(const Sun43DpAD& dp) {
  ChAD omega_bar = cov2cor(dp.Omega);
  ChAD omega_inv = zeros_ad(kSun43D);
  ChAD l{};
  chol_lower_ad(omega_bar, l);
...
std::vector<AD> alpha_from_x(const std::vector<AD>& x, const Sun43DpAD& dp) {
  ChAD omega_bar = cov2cor(dp.Omega);
  ChAD omega_inv = zeros_ad(kSun43D);
  ChAD l{};
  chol_lower_ad(omega_bar, l);
...
AD sun43_log_p2_ad(
  ...
  std::vector<AD> mean_v(kSun43M, AD(0.0));
  std::vector<AD> upper_zero(kSun43M, AD(0.0));
  const AD p2 = detail::pmvn_ad(
    pmvn_call_id, p2_tape, upper_zero, mean_v, scale_g, vech_g);
  return log_prob_floored(p2);
}

void setup_adfun_sparsity(Sun43AdfunPack& pack, const std::vector<double>& par_seed) {
  pack.x_seed = detail::to_cppad_vector(par_seed);
  std::vector<std::size_t> par_subset(kSun43NPar);
...
Sun43Result eval_adfun_log(
  ...
  Sun43Result out;
  out.gradient.assign(kSun43NPar, 0.0);
  out.hessian.assign(kSun43NPar, std::vector<double>(kSun43NPar, 0.0));
  ...
  for (std::size_t j = 0; j < kSun43NPar; ++j) {
    out.gradient[j] += dw[j];
  }
...
void apply_log_scale(Sun43Result& out, bool log_scale, int deriv) {
  if (log_scale) {
    return;
  }
...
Sun43TapeBundle create_sun43_bundle(
  ...
  if (x_rows.empty() || par_seed.size() != kSun43NPar) {
    throw std::runtime_error("invalid seed dimensions for SUN(4,3) tape");
  }
  ...
  Mat lambda = zeros_mat(kSun43M);
  (void)sun43_sigma_double(par_seed, lambda, par_map);
  Mat gamma = zeros_mat(kSun43M);
  {
    std::vector<AD> par_ad(kSun43NPar);
    for (std::size_t i = 0; i < kSun43NPar; ++i) {
      par_ad[i] = AD(par_seed[i]);
  ...
  std::vector<double> lower(kSun43M, kNegInf);
  std::vector<double> mean_zero(kSun43M, 0.0);
  ...
  for (std::size_t obs = 0; obs < x_rows.size(); ++obs) {
    ...
    std::vector<double> upper_alpha(kSun43M);
    for (std::size_t i = 0; i < kSun43M; ++i) {
...
Sun43Result eval_sun43_bundle(
  ...
  Sun43Result total;
  total.gradient.assign(kSun43NPar, 0.0);
  total.hessian.assign(kSun43NPar, std::vector<double>(kSun43NPar, 0.0));
  ...
  if (n_threads > 1 && n_shards > 0) {
    ...
    if (deriv >= 1) {
      shard_grads.assign(n_shards, std::vector<double>(kSun43NPar, 0.0));
    }
    if (deriv >= 2) {
      shard_hess.assign(
        n_shards,
        std::vector<std::vector<double>>(kSun43NPar, std::vector<double>(kSun43NPar, 0.0)));
    }
...
#pragma omp for schedule(static)
      for (std::size_t i = 0; i < n_shards; ++i) {
        Sun43Result shard_res = eval_adfun_log(
    ...
    for (std::size_t i = 0; i < n_shards; ++i) {
      ...
      if (deriv >= 2) {
        for (std::size_t r = 0; r < kSun43NPar; ++r) {
          for (std::size_t c = 0; c < kSun43NPar; ++c) {
  ...
  Sun43Result p2_res = eval_adfun_log(
    bundle.p2.adfun,
