...
using Mat44 = std::vector<std::vector<double>>;

ChAD zeros_ad() {
  return ChAD(kSun44D, std::vector<AD>(kSun44D, AD(0.0)));
}

ChAD identity_ad() {
  ChAD m = zeros_ad();
  for (std::size_t i = 0; i < kSun44D; ++i) {
    m[i][i] = AD(1.0);
  }
  return m;
}
...
ChAD mat_mul(const ChAD& a, const ChAD& b) {
  ChAD out = zeros_ad();
  for (std::size_t i = 0; i < kSun44D; ++i) {
    for (std::size_t j = 0; j < kSun44D; ++j) {
      AD s = AD(0.0);
      for (std::size_t k = 0; k < kSun44D; ++k) {
        s += a[i][k] * b[k][j];
      }
      out[i][j] = s;
    }
...
ChAD unit_row_chol(const std::vector<AD>& z) {
  constexpr std::size_t n_free = kSun44D * (kSun44D - 1) / 2;
  if (z.size() != n_free) {
    throw std::runtime_error("unit_row_chol: expected 6 free parameters");
  }
  ChAD Br = zeros_ad();
  Br[0][0] = AD(1.0);
  std::size_t idx = 0;
  for (std::size_t i = 1; i < kSun44D; ++i) {
    AD sum_sq = AD(0.0);
    for (std::size_t j = 0; j < i; ++j) {
...
struct Sun44DpAD {
  std::vector<AD> xi = std::vector<AD>(kSun44D, AD(0.0));
  ChAD Omega = zeros_ad();
  ChAD Delta = zeros_ad();
  ChAD Gamma = zeros_ad();
};

Sun44DpAD make_sun44_dp_ad(const std::vector<AD>& par) {
  if (par.size() != kSun44NPar) {
    throw std::runtime_error("par must have length 36");
  }
  Sun44DpAD out;
  for (std::size_t i = 0; i < kSun44D; ++i) {
    out.xi[i] = par[i];
  ...
  std::vector<AD> nu{par[4], par[5], par[6], par[7]};
  const std::vector<AD> z_omega{par[8], par[9], par[10],
                                par[11], par[12], par[13]};
  ChAD Bu = unit_row_chol(z_omega);
  ChAD Cu = mat_mul(Bu, mat_transpose(Bu));
  std::vector<AD> s_u(kSun44D);
  for (std::size_t i = 0; i < kSun44D; ++i) {
  ...
  for (std::size_t i = 0; i < kSun44D; ++i) {
    for (std::size_t j = 0; j < kSun44D; ++j) {
      L[i][j] = par[14 + i * kSun44D + j];
    }
  }

  const std::vector<AD> z_gamma{par[30], par[31], par[32],
                                par[33], par[34], par[35]};
...
ChAD unit_row_chol8(const std::vector<AD>& z) {
  if (z.size() != kSun44HsNFree) {
    throw std::runtime_error("unit_row_chol8: expected 28 free parameters");
  }
  ChAD Br(kSun44JointD, std::vector<AD>(kSun44JointD, AD(0.0)));
  Br[0][0] = AD(1.0);
  std::size_t idx = 0;
  const AD kEps = AD(1e-6);
  for (std::size_t i = 1; i < kSun44JointD; ++i) {
    AD sum_sq = AD(0.0);
    for (std::size_t j = 0; j < i; ++j) {
      const AD tij = CppAD::tanh(z[idx++]);
...
Sun44DpAD make_sun44_dp_hs_ad(const std::vector<AD>& par) {
  if (par.size() != kSun44NPar) {
    throw std::runtime_error("par must have length 36");
  }
  Sun44DpAD out;
  ...
  std::vector<AD> z(kSun44HsNFree);
  for (std::size_t i = 0; i < kSun44HsNFree; ++i) {
    z[i] = par[2 * kSun44D + i];
  }
  ChAD B = unit_row_chol8(z);
  ChAD J(kSun44JointD, std::vector<AD>(kSun44JointD, AD(0.0)));
  for (std::size_t i = 0; i < kSun44JointD; ++i) {
    for (std::size_t j = 0; j < kSun44JointD; ++j) {
      for (std::size_t k = 0; k < kSun44JointD; ++k) {
        J[i][j] += B[i][k] * B[j][k];
      }
    }
  }
  for (std::size_t i = 0; i < kSun44D; ++i) {
    const AD si = CppAD::CondExpGt(
      par[kSun44D + i], AD(1e-12), par[kSun44D + i], AD(1e-12));
    for (std::size_t j = 0; j < kSun44D; ++j) {
      const AD sj = CppAD::CondExpGt(
        par[kSun44D + j], AD(1e-12), par[kSun44D + j], AD(1e-12));
      out.Omega[i][j] = si * sj * J[i][j];
      out.Delta[i][j] = J[i][kSun44D + j];
      out.Gamma[i][j] = J[kSun44D + i][kSun44D + j];
    }
  }
  return out;
}

Sun44DpAD make_sun44_dp_dispatch(
  const std::vector<AD>& par, Sun44ParMap par_map) {
  if (par_map == Sun44ParMap::kHyperspherical) {
    return make_sun44_dp_hs_ad(par);
  }
  return make_sun44_dp_ad(par);
}

Mat44 zeros_mat44() {
  return Mat44(kSun44D, std::vector<double>(kSun44D, 0.0));
}

Mat44 sun44_sigma_double(
  const std::vector<double>& par, Mat44& lambda, Sun44ParMap par_map) {
  std::vector<AD> par_ad(kSun44NPar);
  for (std::size_t i = 0; i < kSun44NPar; ++i) {
    par_ad[i] = AD(par[i]);
  }
  Sun44DpAD dp = make_sun44_dp_dispatch(par_ad, par_map);

  ChAD omega_bar = cov2cor(dp.Omega);
  ChAD omega_inv = zeros_ad();
  ChAD l{};
  chol_lower_ad(omega_bar, l);
  for (std::size_t col = 0; col < kSun44D; ++col) {
    std::vector<AD> y(kSun44D, AD(0.0));
    for (std::size_t i = 0; i < kSun44D; ++i) {
    ...
    for (int i = static_cast<int>(kSun44D) - 1; i >= 0; --i) {
      AD s = y[static_cast<std::size_t>(i)];
      for (std::size_t k = static_cast<std::size_t>(i) + 1; k < kSun44D; ++k) {
...
AD log_mvn4(const std::vector<AD>& x, const Sun44DpAD& dp) {
  ChAD l{};
  ...
  return AD(-0.5 * static_cast<double>(kSun44D)) * AD(kLog2Pi)
    - AD(0.5) * log_det - AD(0.5) * quad;
}

ChAD lambda_from_dp(const Sun44DpAD& dp) {
  ChAD omega_bar = cov2cor(dp.Omega);
  ChAD omega_inv = zeros_ad();
  ChAD l{};
  chol_lower_ad(omega_bar, l);
...
std::vector<AD> alpha_from_x(const std::vector<AD>& x, const Sun44DpAD& dp) {
  ChAD omega_bar = cov2cor(dp.Omega);
  ChAD omega_inv = zeros_ad();
  ChAD l{};
  chol_lower_ad(omega_bar, l);
...
void pack_genz_ad(
  ...
  scale.assign(kSun44D, AD(0.0));
  for (std::size_t i = 0; i < kSun44D; ++i) {
    const AD s2 = CppAD::CondExpGt(sigma[i][i], AD(1e-12), sigma[i][i], AD(1e-12));
    scale[i] = CppAD::sqrt(s2);
  }
  vech_ch.assign(vech_size(kSun44D), AD(0.0));
  for (std::size_t i = 0; i < kSun44D; ++i) {
    for (std::size_t j = 0; j <= i; ++j) {
...
AD sun44_log_p2_ad(
  ...
  std::vector<AD> mean_v(kSun44D, AD(0.0));
  std::vector<AD> upper_zero(kSun44D, AD(0.0));
  const AD p2 = detail::pmvn_ad(
    pmvn_call_id, p2_tape, upper_zero, mean_v, scale_g, vech_g);
  return log_prob_floored(p2);
}

void setup_adfun_sparsity(Sun44AdfunPack& pack, const std::vector<double>& par_seed) {
  pack.x_seed = detail::to_cppad_vector(par_seed);
  std::vector<std::size_t> par_subset(kSun44NPar);
...
Sun44Result eval_adfun_log(
  ...
  Sun44Result out;
  out.gradient.assign(kSun44NPar, 0.0);
  out.hessian.assign(kSun44NPar, std::vector<double>(kSun44NPar, 0.0));
  ...
  for (std::size_t j = 0; j < kSun44NPar; ++j) {
    out.gradient[j] += dw[j];
  }
...
void apply_log_scale(Sun44Result& out, bool log_scale, int deriv) {
  if (log_scale) {
    return;
  }
...
Sun44TapeBundle create_sun44_bundle(
  ...
  if (x_rows.empty() || par_seed.size() != kSun44NPar) {
    throw std::runtime_error("invalid seed dimensions for SUN(4,4) tape");
  }
  ...
  Mat44 lambda = zeros_mat44();
  (void)sun44_sigma_double(par_seed, lambda, par_map);
  Mat44 gamma = zeros_mat44();
  {
    std::vector<AD> par_ad(kSun44NPar);
    for (std::size_t i = 0; i < kSun44NPar; ++i) {
      par_ad[i] = AD(par_seed[i]);
  ...
  std::vector<double> lower(kSun44D, kNegInf);
  std::vector<double> mean_zero(kSun44D, 0.0);
  ...
  for (std::size_t obs = 0; obs < x_rows.size(); ++obs) {
    ...
    std::vector<double> upper_alpha(kSun44D);
    for (std::size_t i = 0; i < kSun44D; ++i) {
...
Sun44Result eval_sun44_bundle(
  ...
  Sun44Result total;
  total.gradient.assign(kSun44NPar, 0.0);
  total.hessian.assign(kSun44NPar, std::vector<double>(kSun44NPar, 0.0));
  ...
  if (n_threads > 1 && n_shards > 0) {
    ...
    if (deriv >= 1) {
      shard_grads.assign(n_shards, std::vector<double>(kSun44NPar, 0.0));
    }
    if (deriv >= 2) {
      shard_hess.assign(
        n_shards,
        std::vector<std::vector<double>>(kSun44NPar, std::vector<double>(kSun44NPar, 0.0)));
    }
...
#pragma omp for schedule(static)
      for (std::size_t i = 0; i < n_shards; ++i) {
        Sun44Result shard_res = eval_adfun_log(
    ...
    for (std::size_t i = 0; i < n_shards; ++i) {
      ...
      if (deriv >= 2) {
        for (std::size_t r = 0; r < kSun44NPar; ++r) {
          for (std::size_t c = 0; c < kSun44NPar; ++c) {
  ...
  Sun44Result p2_res = eval_adfun_log(
    bundle.p2.adfun,
