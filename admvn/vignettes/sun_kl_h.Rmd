---
title: "SUN(3,3) KL matching with joint hyperspherical parameterization"
author: "Patrick Brown"
date: "`r Sys.Date()`"
output: pdf_document
vignette: >
  %\VignetteIndexEntry{SUN(3,3) KL matching with joint hyperspherical parameterization}
...
This vignette mirrors `sun_kl`, but uses the joint hyperspherical
parameterization. It builds a known SUN$(3,3)$, forms a weighted discrete
target from it, and minimises the KL / cross-entropy with
`fit_sun33_hs_quad()`. The fitting score uses the `dsun_hs_fun()` tape
(same as `evaluate_sun33_hs_slice()`). Soft boxes come from
`sun33_hs_bounds()`. See `sun_kl` for the block-Cholesky analogue.

Because several joint-correlation coordinates can represent nearly the same
SUN law, raw recovery of all 21 free parameters is often weak. We therefore
check that the KL drops toward the value at the true parameters, that the
fitted density matches on a hold-out sample, and that the `sn`-style `dp`
summaries are close.

## True SUN(3,3)

Mild $U$-block correlation, independent-SN pair slots, and a few small
cross / $V$ coordinates:
...
(`make_sun33_hs_start_from_normal()` puts $C_u$ free coordinates from
`cov2cor(Sigma)` and optional pair-slot skew).
...
fitted `dp` should nearly match the true SUN. Individual free $z$ coordinates
may still differ when they are only weakly identified; agreement of the
implied density and of `Omega` / `Delta` / `Gamma` is the more meaningful
recovery check.
...
```{r kl-sun33-hs-profile-setup, eval = requireNamespace("sn", quietly = TRUE)}
...
```{r kl-sun33-hs-profile-1, eval = requireNamespace("sn", quietly = TRUE), fig.width = 7, fig.height = 9, fig.cap = "SUN(3,3) hs slice profiles for parameters 1-7 (cross-entropy objective; FD vs AD gradient)."}
profile_par_block(1:7)
```

```{r kl-sun33-hs-profile-2, eval = requireNamespace("sn", quietly = TRUE), fig.width = 7, fig.height = 9, fig.cap = "SUN(3,3) hs slice profiles for parameters 8-14."}
profile_par_block(8:14)
```

```{r kl-sun33-hs-profile-3, eval = requireNamespace("sn", quietly = TRUE), fig.width = 7, fig.height = 9, fig.cap = "SUN(3,3) hs slice profiles for parameters 15-21."}
