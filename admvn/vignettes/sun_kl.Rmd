---
title: "SUN(3,3) KL matching and recovery"
author: "Patrick Brown"
date: "`r Sys.Date()`"
output: pdf_document
vignette: >
  %\VignetteIndexEntry{SUN(3,3) KL matching and recovery}
...
This vignette builds a known SUN$(3,3)$, forms a weighted discrete target from
it, and minimises the KL / cross-entropy with `fit_sun33_quad()`. The fitting
score uses the `dsun_fun()` tape (same as `evaluate_sun33_slice()`).
The free parameters use the residual-correlation construction so that
$\operatorname{Var}(V)=\Sigma_{vv}$ is a correlation matrix
(`diag(Gamma) = 1`) without an $\omega_v$ rescale. Skew loadings $L$ act on
the correlation $C_u$ of the standardized latent (scale
$D_u=\operatorname{diag}(\nu)$ with $\nu_j$ the marginal standard deviations
is applied outside), so $|L_{ij}|\le 1$ is the natural regime.

Because several block-Cholesky coordinates can represent nearly the same SUN
law, raw recovery of all 21 free parameters is often weak (especially the
off-diagonal contrasts of $L$). We therefore check
that the KL drops toward the value at the true parameters, that the fitted
density matches on a hold-out sample, and that the `sn`-style `dp` summaries
are close.
...
## Discrete target from the true law

Sample from the true SUN and use the empirical measure as $p$, with `logp`
from `sn::dsun` at the true `dp`.

```{r target, eval = requireNamespace("sn", quietly = TRUE)}
library(sn)
set.seed(1)
n <- 600L
...
Start from a nearly Gaussian SUN matched to the sample moments.

```{r start, eval = requireNamespace("sn", quietly = TRUE)}
...
```{r fit, eval = requireNamespace("sn", quietly = TRUE)}
fit <- fit_sun33_quad(
  quad = quad,
  start = par_start,
  mc.cores = 1L,
...
```{r fig-recovery, eval = requireNamespace("sn", quietly = TRUE), fig.height = 4}
...
**What success looks like.** The fitted KL should be much smaller than at the
start and close to the KL at `par_true`. Hold-out mean log-density under the
fitted `dp` should nearly match the true SUN. Individual free parameters
(especially $L$ and the latent-correlation coordinates) may still differ when
they are only weakly identified; agreement of the implied density and of
`Omega` / `Delta` / `Gamma` is the more meaningful recovery check.

## Parameter profiles: AD vs finite-difference gradients

Slice the cross-entropy objective $-\sum_i w_i\log q(x_i)$ one coordinate at a
time around the fitted value, and compare the AD partial derivative to a
central finite-difference (Richardson) estimate on the same grid.

```{r kl-sun33-profile-setup, eval = requireNamespace("sn", quietly = TRUE)}
...
```{r kl-sun33-profile-1, eval = requireNamespace("sn", quietly = TRUE), fig.width = 7, fig.height = 9, fig.cap = "SUN(3,3) slice profiles for parameters 1-7 (cross-entropy objective; FD vs AD gradient)."}
profile_par_block(1:7)
```

```{r kl-sun33-profile-2, eval = requireNamespace("sn", quietly = TRUE), fig.width = 7, fig.height = 9, fig.cap = "SUN(3,3) slice profiles for parameters 8-14."}
profile_par_block(8:14)
```

```{r kl-sun33-profile-3, eval = requireNamespace("sn", quietly = TRUE), fig.width = 7, fig.height = 9, fig.cap = "SUN(3,3) slice profiles for parameters 15-21."}
profile_par_block(15:21)
```

For each coordinate the left panel is the profiled objective and the right
panel overlays the finite-difference slope (grey crosses) and the AD
partial (blue pluses). Agreement of the two gradient curves is the gradient
check; a vertical line marks the fitted value.
