...
.evaluate_sun_slice <- function(par, quad,
  ...
  if (family == "22") {
    ...
    if (length(par) != 23L) {
      stop("par must have length 23 for SUN(4,2) hyperspherical")
    }
    if (is.null(n_points)) n_points <- 64L
    if (is.null(n_shifts)) n_shifts <- 2L
    tape <- tryCatch(
...
.fit_sun_quad <- function(quad,
  ...
  if (family == "22") {
    ...
    if (npar != 23L) {
      stop("start must have length 23 for SUN(4,2) hyperspherical")
    }
...
#' Slice KL / cross-entropy of a SUN(4,2) hyperspherical fit
#' @inheritParams evaluate_sun33_hs_slice
#' @export
evaluate_sun42_hs_slice <- function(par, quad,
                                    n_points = 64L,
                                    n_shifts = 2L,
                                    n_threads = 1L) {
  .evaluate_sun_slice(
...
#' Fit SUN(4,2) hyperspherical by minimising KL on a weighted quadrature
#'
#' @inheritParams fit_sun33_hs_quad
#' @param start Length-23 hyperspherical start
#'   (see [make_sun42_hs_start_from_normal()]).
#' @param lower,upper Box constraints (defaults from [sun42_hs_bounds()]).
#' @export
fit_sun42_hs_quad <- function(quad,
                              start,
                              lower = NULL,
                              upper = NULL,
                              obj_scale = 1e4,
