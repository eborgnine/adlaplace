#' SUN(4,3) density with joint hyperspherical parameterization
#'
#' @param x Numeric matrix with 4 columns (observations in rows).
#' @param par Numeric vector of length 29; see [make_sun43_hs_params()].
...
#' @export
dsun43_hs <- function(x, par, log = TRUE, deriv = 0L, n_points = 1021L,
                      n_shifts = 8L, seed = 1L, n_threads = 1L,
                      weights = NULL) {
  par <- as.numeric(par)
  if (length(par) != 29L) stop("par must have length 29 for SUN(4,3) hs")
  deriv <- as.integer(deriv)
  if (!deriv %in% 0L:2L) stop("deriv must be 0, 1, or 2")
  x <- as.matrix(x)
  if (ncol(x) != 4L) stop("x must have 4 columns for SUN(4,3)")
  if (!is.null(weights)) {
  ...
  if (!nrow(x)) {
    ...
    if (deriv >= 1L) out$gradient <- numeric(29L)
    if (deriv >= 2L) out$hessian <- matrix(0, 29L, 29L)
    return(out)
  }
...
#' Create a reusable SUN(4,3) hyperspherical likelihood tape
#'
#' @param x Numeric matrix with 4 columns.
#' @param par_seed Seed vector for [make_sun43_hs_params()].
#' @param n_points,n_shifts Orthant QMC settings.
#' @param seed QMC seed.
#' @param n_threads OpenMP threads (default from package helper).
#' @param weights Optional observation weights.
#' @export
dsun43_hs_fun <- function(x, par_seed, n_points = 1021L, n_shifts = 8L,
                          seed = 1L, n_threads = NULL, weights = NULL) {
  par_seed <- as.numeric(par_seed)
  if (length(par_seed) != 29L) stop("par_seed must have length 29 for SUN(4,3) hs")
  x <- as.matrix(x)
