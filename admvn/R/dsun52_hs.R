#' SUN(5,2) density with joint hyperspherical parameterization
#'
#' @param x Numeric matrix with 5 columns (observations in rows).
#' @param par Numeric vector of length 31; see [make_sun52_hs_params()].
...
#' @export
dsun52_hs <- function(x, par, log = TRUE, deriv = 0L, n_points = 1021L,
                      n_shifts = 8L, seed = 1L, n_threads = 1L,
                      weights = NULL) {
  par <- as.numeric(par)
  if (length(par) != 31L) stop("par must have length 31 for SUN(5,2) hs")
  deriv <- as.integer(deriv)
  if (!deriv %in% 0L:2L) stop("deriv must be 0, 1, or 2")
  x <- as.matrix(x)
  if (ncol(x) != 5L) stop("x must have 5 columns for SUN(5,2)")
  if (!is.null(weights)) {
  ...
  if (!nrow(x)) {
    ...
    if (deriv >= 1L) out$gradient <- numeric(31L)
    if (deriv >= 2L) out$hessian <- matrix(0, 31L, 31L)
    return(out)
  }
...
#' Create a reusable SUN(5,2) hyperspherical likelihood tape
#'
#' @param x Numeric matrix with 5 columns.
#' @param par_seed Seed vector for [make_sun52_hs_params()].
#' @param n_points,n_shifts Orthant QMC settings.
#' @param seed QMC seed.
#' @param n_threads OpenMP threads (default from package helper).
#' @param weights Optional observation weights.
#' @export
dsun52_hs_fun <- function(x, par_seed, n_points = 1021L, n_shifts = 8L,
                          seed = 1L, n_threads = NULL, weights = NULL) {
  par_seed <- as.numeric(par_seed)
  if (length(par_seed) != 31L) stop("par_seed must have length 31 for SUN(5,2) hs")
  x <- as.matrix(x)
  if (ncol(x) != 5L || nrow(x) < 1L) stop("x has invalid dimensions")
  storage.mode(x) <- "double"
  if (!is.null(weights)) {
    weights <- as.numeric(weights)
    if (length(weights) != nrow(x)) stop("weights must have length nrow(x)")
