...
test_that("cond_sim_iwp builds summaries from a minimal fit object", {
  skip_if_not_installed("adlaplace")
  ...
  fit <- list(
    objects = list(
      ...
      parameters_info = list(
        gamma = random_info(iwp_term, data)
      )
    ),
    parameters = list(
      gamma = data.frame(
        mode = rep(0, nrow(random_info(iwp_term, data))),
        gamma_label = random_info(iwp_term, data)$gamma_label,
        stringsAsFactors = FALSE
      ),
      beta = data.frame(
        mle = numeric(0),
        beta_label = character(0),
        stringsAsFactors = FALSE
      )
    ),
    extra = list(
      half_H_inv = Matrix::Diagonal(nrow(random_info(iwp_term, data)), 0.5)
    )
  )

  result <- cond_sim_iwp(fit, n = 5L)
