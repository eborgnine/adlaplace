...
are built via `adlaplace::ad_pack_ptr()` using the `package` slot on each `density_data`
...
config <- list(
  ...
  obs_groups = adlaplace::obs_groups(Amat, num_groups = 100),
  num_threads = 4L,
  verbose = FALSE
)
...
Shard handles are combined with `c()`. Clone before multi-thread `ad_pack()` so
serial dens stays on a plain handle; after `ad_pack()` the moved inputs must not
be reused.
...
dens <- vapply(
  ...
  adlaplace::joint_log_dens,
  numeric(1),
  ad_pack = ad_fun_plain, ad_shards = shards,
  negative = FALSE
)
...
grad <- do.call(
  ...
  lapply(
    x_list,
    adlaplace::grad,
    ad_pack = ad_fun_plain,
    inner = FALSE, ad_shards = shards,
    negative = FALSE
  )
)
...
md$term_data$info$theta[, c("label", "log", "id")]
...
ad_extra <- adlaplace::ad_pack_ptr(data = data_extra, config = config)
ad_extra2 <- adlaplace::ad_pack_ptr(data = md$parameters$y_extra, config = config2)
ad_pack2 <- adlaplace::ad_pack(
  md,
  config2,
  num_threads = 1L
)
...
adlaplace::joint_log_dens(ad_pack2, xx_all2,
  ad_shards = adlaplace:::n_groups(ad_pack2@ptr) - 1L,
  negative = FALSE
)
...
by_shard <- vapply(
  ...
  function(s) adlaplace::joint_log_dens(ad_fun_plain, xx_all, ad_shards = s, negative = FALSE),
  numeric(1)
)
by_shard2 <- vapply(
  seq.int(from = 0L, length.out = adlaplace:::n_groups(ad_pack2@ptr)),
  function(s) adlaplace::joint_log_dens(ad_pack2, xx_all2, ad_shards = s, negative = FALSE),
  numeric(1)
)
...
rbind(log_lik2$deriv$d_neg_log_lik, log_lik1$deriv$d_neg_log_lik[param_remap])
```
