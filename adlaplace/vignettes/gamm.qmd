...
res <- adlaplace::log_lik_laplace(
  x = c(md$data$info$beta$init, log(md$data$info$theta$init)),
  ad_fun = ad_fun,
...
config$opt <- as.list(
  rbind(
    md$data$info$beta[, to_keep],
    theta_for_opt[, to_keep]
  )
)
config$opt$lower[is.infinite(config$opt$lower)] <- -5
config$opt$upper[is.infinite(config$opt$upper)] <- 5
