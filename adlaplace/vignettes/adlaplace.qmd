...
adFun <- adlaplace::build_adfun(data, modifyList(config, list(verbose = TRUE)))
...
dens1 <- mapply(
  ...
  x = as.list(parDf),
  MoreArgs = list(ad_fun = adFun, Sgroups = Sgroups),
  SIMPLIFY = TRUE
)
...
grad1 <- mapply(
  adlaplace::grad,
  x = as.list(parDf),
  MoreArgs = list(ad_fun = adFun, inner = inner, Sgroups = Sgroups),
  SIMPLIFY = TRUE
)
plot(Sx, grad1[Dpar, ])
...
my_derivs <- adlaplace::all_derivs(x, adFun, config)

c(my_derivs$fval, -adlaplace::jointLogDens(adFun, x))

my_grad <- adlaplace::grad(adFun, x)
rbind(my_grad, my_derivs$gradient)[, 1:5]

my_hessian <- adlaplace::hessian(adFun, x)
max(abs(my_hessian@x + my_derivs$hessian@x))
...
Sdet <- unlist(lapply(res, function(xx) {
  Matrix::determinant(
    xx$hessian$chol_inner,
    sqrt = TRUE, logarithm = TRUE
  )$modulus
}))
dU <- do.call(
  abind::abind,
...
str(adlaplace::grad(adFun, x, inner = FALSE))
str(adlaplace::grad(adFun, x, inner = TRUE))
