...
overdispersion parameter. Densities are implemented in `src/nbinom.cpp` (observations
and hyperparameters) and `src/random.cpp` (random effects).
...
ad_pack <- adlaplace::ad_fun(ad_fun_plain, num_threads = 4)
```
...
res_fdf <- fun_obj_fdfh(ad_pack, c(config$beta, config$theta), config$gamma, inner = TRUE, verbose = TRUE)

c(
  sum(by_shard),
  adlaplace::joint_log_dens(ad_fun_plain, x_full, negative = FALSE),
  adlaplace::joint_log_dens(ad_pack, x_full, negative = FALSE),
  -res_fdf$f
)
```
