...
```{r testLogDens}
ad_fun_plain <- adlaplace::get_ad_fun_raw(data, config)
...
ad_fun_plain <- adlaplace::get_ad_fun_raw(data, modifyList(config, list(verbose = TRUE)))
...
dens1 <- mapply(
  ...
  x = as.list(parDf),
  MoreArgs = list(ad_fun = ad_fun_plain, Sgroups = Sgroups),
  SIMPLIFY = TRUE
)
...
grad1 <- mapply(
  adlaplace::grad,
  x = as.list(parDf),
  MoreArgs = list(ad_fun = ad_fun_plain, inner = inner, Sgroups = Sgroups),
  SIMPLIFY = TRUE
)
plot(Sx, grad1[Dpar, ])
...
quantile(inner_res$chol_inner$D - chol_test$D@x)
```
