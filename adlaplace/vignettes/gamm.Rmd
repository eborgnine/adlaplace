...
```{r gamm}
# Use packaged data for vignette builds (stable, no mgcv::gamm during render).
...
#   b2 <- mgcv::gamm(y ~ x1 + s(x2), family = mgcv::nb,
#                    data = dat, random = list(fac = ~1))
#   save(dat, b2, file = "data/gamm.RData")
...
```{r ad_fun_create}
# Serial for vignette builds: OpenMP TLS can segfault under some macOS
# libomp / dyn.load combinations when num_threads > 1 (vignette build then dies
# at log_lik with no R error message).
config$num_threads <- 1L
