...
```{r precision}
ref_id <- which.max(apply(germany$adj, 1, sum))
idx <- setdiff(seq_len(n), ref_id)

deg <- Matrix::rowSums(germany$adj)
Q_icar <- Matrix::Diagonal(n, deg) - germany$adj
...
$2n - 1$: structured effects on $n - 1$ areas (reference area `ref_id` fixed
at zero) plus $n$ iid effects. Observations use `poisson_obs` with offset
$\log E_i$.
...
ad_pack <- adlaplace::ad_fun(ad_fun_plain, num_threads = 4L)
```
...
plot(
  ...
  main = "Total spatial effect (adlaplace mode)"
)
```
