...
`deriv = TRUE`, and `trace_hinv_t` (same OpenMP shard groups as `inner_opt`, no
separate serial driver). `inner_opt()` uses one OpenMP/CppAD parallel session for
trust-region inner optimization and outer `get_fdfh` when `deriv = TRUE`.
...
`config$verbose = TRUE`, `inner_opt` and `trace_hinv_t` print the same style of
CppAD session phase messages (trust region, outer `get_fdfh`, trace shards).
Each `CppadParallelScope` end flushes per-thread CppAD pools before the next
`.Call` (e.g. `trace_hinv_t` after R work in `log_lik_deriv`, or another optimizer step).
