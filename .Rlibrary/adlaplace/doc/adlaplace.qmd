...
```{r testLogDens}
ad_fun <- adlaplace::build_adfun(data, config)

x <- c(config$beta, rep(0.1, length(config$gamma)), config$theta)
s_groups <- 0:3
adlaplace::jointLogDens(ad_fun, x, s_groups)
(my_grad <- adlaplace::grad(ad_fun, x, Sgroups = s_groups))
my_hess3 <- adlaplace::hessian(ad_fun, x, Sgroups = s_groups)
```
