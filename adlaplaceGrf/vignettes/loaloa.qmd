...
```{r model_setup}
#| cache: false
knots_grid <- geostatsp::squareRaster(loaloa, cells = 8, buffer = 500 * 1000)
...
plot_compare <- has_INLA && exists("loaFit") && length(loaFit$parameters)
plot_alone <- !plot_compare
```

```{r plot-surfaces-compare}
#| fig-width: 8
#| fig-height: 8
#| layout-ncol: 2
#| fig-cap: "Posterior mean and SD of the spatial field (glgm vs adlaplace)."
#| fig-subcap:
#|   - "glgm random mean"
#|   - "adlaplace matern mean"
#|   - "glgm random sd"
#|   - "adlaplace matern sd"
