...
wires it into `adlaplace` via `random_fem_2`, and evaluates the joint
...
spline weights $w$ (`random_fem_2`). Hyperparameters are
$(\log\sigma, \log\kappa, \log\tau)$.
...
# Parameter layout: gamma (nr), theta = (log_sigma, log_kappa, log_tau)
...
Along one coordinate (here $\log\kappa$), compare the AD gradient to a
...
par(mfrow = c(2, 2), mar = c(2.5, 2.5, 1.5, 0.5), mgp = c(1.4, 0.5, 0))

Dpar <- length(x) - 1L # log kappa
Ngrid <- 11L
...
plot(Sx, dens_scan, type = "l", xlab = expression(log(kappa)), ylab = "log dens")
plot(Sx, grad_scan[Dpar, ], type = "l", xlab = expression(log(kappa)), ylab = "AD gradient")
...
# Hessian row for log kappa vs a weight and vs log tau
for (Dpar2 in c(1L, length(x))) {
  plot(
    Sx, hes_scan[Dpar, Dpar2, ],
    type = "l",
    xlab = expression(log(kappa)),
...
# FD Hessian column for log kappa from AD gradients
j <- Dpar
